...
\begin{document}
    ...
    Traditional SAR methods face substantial limitations, such as limited search coverage, difficulties in maintaining full readiness, high operational costs, extended response times, and coordination challenges in dynamic environments~\cite{sar_challenges}.
    ...
    Prior research states that, in Riverine MSAR operations~\cite{riverine_MSAR_stress}, crew tend to suffer marked increases in response time, and difficulties locating targets.
    ...
    Research has demonstrated that autonomous UAVs can greatly benefit Search and Rescue operations, though it was used in SAR operations over land~\cite{waharte2010supporting}.
    ...
    Recent work in the 2020s ~\cite{messmer2024uav,UAV_USV_SAR} expanded this to practical MSAR implementations, developing a comprehensive software framework combining onboard UAV detection with ground-station processing to reduce operator strain.
    ...
    A dual-layer task planning framework proposed by recent research~\cite{YANG2025113488} integrates UAV search capabilities with human-crewed execution, in complex indoor environments.
    ...
    Finally, research exists proposing the design of an Autonomous Unmanned Vessel directly intended for MSAR missions, indicating that practical implementation is viable if certain questions around coordination can be solved.
    ...
    Although intended more for Computer Animation, Reynolds' seminal work in ~\cite{10.1145/37402.37406} helped inform the basis for most swarm coordination technology.
    ...
    Systems like EN-MASCA~\cite{ENMASCA2025}, designed for Durian Orchards, represent a real-world implementation of Autonomous Drone Swarm Coordination.
    The proposed system, EN-MASCA, demonstrated the application of Reinforcement Learning approaches to optimize formations and obstacle avoidance.
    ...
    Another algorithm named SwarmRaft~\cite{dev2025swarnraft} utilizes a consensus-based approach to collectively decide on the current state for real-time swarm coordination in GNSS-denied environments.
    By leveraging distributed consensus mechanisms, SwarmRaft allows drones to handle the inevitable connection issues that they will experience at sea, even when individual units experience sensor degradation or sensor loss.
    ...
    In a discussion centered on Situation Awareness in the context of flight systems, Endsley said in 2021~\cite{situation_awareness} that, counterintuitively, passively monitoring highly automated systems have the potential to increase mental workload rather than decrease it.
    ...
    Cummings~\cite{CUMMINGS2007339}, in 2007, conducted extensive research on operator workload limits in Human UAV supervision.
    ...
    A cognitive model of UAV control set out by researchers at the 16th International Symposium on Aviation Psychology~\cite{wickens2011cognitive} reveals that workload tends to ``spike`` right before, and during high-task phases of a mission.
    ...
    In addition, however, there is also limited work validating the effectiveness of human-swarm interface design through objective metrics like NASA-TLX ~\cite{HART1988139}.

    The project seeks to address these gaps through an integrated framework combining proven techniques from each domain, optimized specifically for managing cognitive loads during MSAR operations.
