...
pdflatex -interaction=nonstopmode -halt-on-error interim_report.tex 2>&1 | tee "$LOG_DIR/pdflatex-pass1.scripts.log"
if [ -f interim_report.aux ]; then
  bibtex interim_report 2>&1 | tee "$LOG_DIR/bibtex.scripts.log"
fi
pdflatex -interaction=nonstopmode -halt-on-error interim_report.tex 2>&1 | tee "$LOG_DIR/pdflatex-pass2.scripts.log"
pdflatex -interaction=nonstopmode -halt-on-error interim_report.tex 2>&1 | tee "$LOG_DIR/pdflatex-pass3.scripts.log"
