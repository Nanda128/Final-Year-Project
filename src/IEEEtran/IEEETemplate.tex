...
\begin{document}
    ...
    For more information about how to become an IEEE Conference author or how to write your paper, please visit   IEEE Conference Author Center website: https://conferences.ieeeauthorcenter.ieee.org/.

    \subsection{Maintaining the Integrity of the Specifications}\label{subsec:maintaining-the-integrity-of-the-specifications}
    ...
    formatting. Please note sections \ref{AA} to \ref{subsec:fat} below for more information on
    ...
    subsubsection and a table, you might find that Table I have been cross-referenced as Table IV-B3.
    ...
    \subsection{Figures and Tables}\label{subsec:fat}
    ...
    Use the abbreviation ``Fig.~\ref{fig:fig}'', even at the beginning of a sentence.
    ...
    As an example, write the quantity
    ``Magnetization'', or ``Magnetization, M'', not just ``M''.
    If including
    units in the label, present them within parentheses.
    Do not label axes only
    with units.
    In the example, write ``Magnetization (A/m)'' or ``Magnetization
    \{A[m(1)]\}'', not just ``A/m''.
    Do not label axes with a ratio of
    quantities and units.
    For example, write ``Temperature (K)'', not
    ``Temperature/K''.
